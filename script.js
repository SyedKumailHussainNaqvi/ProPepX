...
// Hover uses CSS scale; click opens full-screen preview; outside click/Esc closes.
(function () {
  const lightbox = document.getElementById('figureLightbox');
  const lightboxImage = document.getElementById('lightboxImage');
  const lightboxTitle = document.getElementById('lightboxTitle');
  const openOriginal = document.getElementById('openOriginalFigure');

  if (!lightbox || !lightboxImage || !openOriginal) return;
  ...
  const openLightbox = (img) => {
    ...
    const title = img.dataset.zoomTitle || img.alt || 'Full-size figure';

    lightboxImage.src = absoluteUrl;
    lightboxImage.alt = img.alt || title;
    lightboxTitle.textContent = title;
    openOriginal.href = absoluteUrl;
  ...
  const closeLightbox = () => {
    ...
    // Clear after the fade/display cycle so repeated clicks always reload cleanly.
    window.setTimeout(() => {
      if (!lightbox.classList.contains('active')) lightboxImage.removeAttribute('src');
    }, 120);
  };

  document.querySelectorAll('img[data-zoom="true"]').forEach((img) => {
    img.setAttribute('tabindex', '0');
    img.setAttribute('role', 'button');
    img.setAttribute('aria-label', 'Open full-size figure preview');
